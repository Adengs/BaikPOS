...
public class EditProdukDialogFragment extends DialogFragment {
    ...
    @BindView(R.id.layStock)
    TextInputLayout layStock;
    ...
    File imageFoto;
    private GetProducts.DATABean dataProduct;


    public static EditProdukDialogFragment newInstance(GetProducts.DATABean data) {


        EditProdukDialogFragment fragment = new EditProdukDialogFragment();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        layStock.setVisibility(View.GONE);
        txtTitle.setText(getString(R.string.edit_produk));
    ...
    private void setDataProduct() {
        txtNamaProduk.setText(dataProduct.getItemName());
        txtHargaModal.setText(dataProduct.getItemHargaModal().replace(".00", ""));
        txtHargaJual.setText(dataProduct.getItemHargaJual().replace(".00", ""));
        Picasso.get().load(dataProduct.getItemImage()).into(ivProduk);
    }
    ...
    private void initAdapterUnits(GetUnits response) {
        ...
        for (GetUnits.DATAUnits cat : unitList) {
            if (cat.getUnit_id() == dataProduct.getUnit().getUnitId()) {
        ...
        spinUnit.setSelection(selectedPosition);
    }
    ...
    private void initAdapterCategory(GetProductCategory response) {
        ...
        for (GetProductCategory.DATA cat : categoryList) {
            if (cat.getItemCatName().equalsIgnoreCase(dataProduct.getCategoryNameTxt())) {
                selectedPosition = position;
            }
        ...
        spinKategori.setSelection(selectedPosition);
    }
...
//            selectedImage = Base64.encodeToString(byteArray, Base64.DEFAULT);
    ...
    public void onSubmitClicked() {
        ...
        if (imageFoto != null) {
            ...
            RequestBody requestFile = RequestBody.create(MediaType.parse("image/*"), imageFoto);
            body = MultipartBody.Part.createFormData("product_image", imageFoto.getName(), requestFile);
        }
        RetrofitInterface apiService = ApiUtils.getAPIService();
        Call<AddProduct> call = apiService.editProduct( auth, dataProduct.getItemId() + "",param, body);
